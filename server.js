// server.js - Multi-Room Chat Server (Homework 3)
// Team:
//   Sharifova Durdona Uchkun Kizi (24013572)  - Server Core Developer
//   Bobokhonova Dilshoda (23013076)          - Room & User Management
//   Tillabaev Yosinbek (25013516)           - Client Application Developer
//   Olimjonov Hojiakbar (23012976)          - Testing & Documentation Lead
...
io.on("connection", (socket) => {
    ...
    socket.emit(
        "server-message",
        `Welcome ${defaultNick}. Use NICK, JOIN, MSG.`
    );

    socket.on("command", (line = "") => {
        try {
            handleCommand(socket, String(line));
        } catch (err) {
...
function handleCommand(socket, line) {
    ...
    if (cmd === "NICK") {
        ...
        if (!newNick)
            return socket.emit("server-message", "Usage: NICK <nickname>");
        const old = info.nick;
    ...
    if (cmd === "MSG") {
        ...
        if (!room || !text)
            return socket.emit("server-message", "Usage: MSG <room> <message>");
        if (info.room !== room)
            return socket.emit(
                "server-message",
                `You are not in ${room}. JOIN first.`
            );
        if (text.length > MAX_MSG_LEN)
            return socket.emit(
                "server-message",
                `Message too long (max ${MAX_MSG_LEN})`
            );
    ...
    if (cmd === "REACT") {
        ...
        if (!info.room)
            return socket.emit("server-message", "Join a room first");
    ...
    if (cmd === "WHOIS") {
        ...
        if (!targetNick)
            return socket.emit("server-message", "Usage: WHOIS <nickname>");
        ...
        if (foundInfo) {
            if (foundInfo.room) {
                ...
                socket.emit(
                    "server-message",
                    `${targetNick} is currently in room: ${foundInfo.room}`
                );
            } else {
                // Agar xonada bo'lmasa, lekin ulangan bo'lsa
                socket.emit(
                    "server-message",
                    `${targetNick} is connected but not in a room.`
                );
            }
        } else {
    ...
    socket.emit(
        "server-message",
        `Unknown command: ${cmd}. Use NICK, JOIN, MSG.`
    );
}
